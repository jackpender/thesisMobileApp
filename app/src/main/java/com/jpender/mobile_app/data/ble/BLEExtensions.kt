...
fun BluetoothGatt.printGattTable() {
    //If statement to check for the services
    if (services.isEmpty()) {
        Log.d("BluetoothGatt","No service and characteristic available, call discoverServices() first?")
        return
    }
    services.forEach { service -> //Checking for the service of each BLE device
        val characteristicsTable = service.characteristics.joinToString( //Checking for the characteristics of each service
            separator = "\n|--",
            prefix = "|--"
        ) { char -> //For each characteristic there is a description and descriptors
